...
import com.yongchun.library.utils.ImageUtil;
import com.yongchun.library.utils.ImageUtil.DisplayOptions;
...
public class ImageListAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public void onBindViewHolder(final RecyclerView.ViewHolder holder, final int position) {
        if (getItemViewType(position) == TYPE_CAMERA) {
            ...
            options.errorIcon=R.drawable.image_placeholder;
            options.thumbnail=0.5f;
            options.animateMode=DisplayOptions.MODE_ANIMATE_NOT;
            options.isScale=true;
            options.scaleType=ScaleType.CENTER_CROP;
            ImageUtil.get().display(contentHolder.picture, image.getPath(), options);
//            Glide.with(context)
